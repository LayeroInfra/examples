...
## Деплой — работает, но проверяй раннера ПЕРЕД пушем

`deploy-examples.yml` и `deploy-static.yml` идут на
`runs-on: [self-hosted, layero-builder]` — раннер живёт на узле флота.

Раннеры вернулись 16.08.2026 (`T-20260816-6` закрыт): последний зелёный
прогон — 17.08. Прежняя редакция этого файла утверждала «offline, push не
выкатывается», и это отговаривало пушить — при живом CI.

🚨 **Но узлы флота прерываемые, и замена узла раннера уносит.** Отказ при
этом тихий: job уходит в `queued`, а не в `failure`, и висит до
24-часового таймаута. Так CI простоял незамеченным с 12 по 16.08, и так же
он стоял час 24.08.2026 — уже по другой причине.

Поэтому перед пушем сюда:

```bash
CHECK=1 bash ../core/deploy/setup-ci-runner.sh   # ⬅ секунда, отвечает по всем репозиториям
bash ../core/deploy/setup-ci-runner.sh examples  # если раннера нет
```

И после пуша смотреть не статус прогона, а живой адрес примера.
